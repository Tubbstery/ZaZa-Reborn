...
void Colorpicker::think( ) {
	...
	if (g_input.IsCursorInRect(preview)) {
		if (g_input.GetKeyPress(VK_MBUTTON)) {
			g_cl.m_copied_color = m_color;
			g_notify.add(tfm::format(XOR("copied color (%i, %i, %i)\n"), m_color.r(), m_color.g(), m_color.b()));
		}
		if (g_input.GetKeyPress(VK_RBUTTON)) {
			if (!&g_cl.m_copied_color) {
				g_notify.add(XOR("invalid color\n"));
				return;
			}
			m_color = g_cl.m_copied_color;
			g_notify.add(tfm::format(XOR("pasted color (%i, %i, %i)\n"), m_color.r(), m_color.g(), m_color.b()));
		}
	}
