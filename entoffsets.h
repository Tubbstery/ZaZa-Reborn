...
public:
    ...
    void init( ) {
        ...
        INIT_XOREDNETVAR_SPECIFIC( m_nCachedBonesPosition,       *(int*)(pattern::find(g_csgo.m_client_dll, XOR("8D 87 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 8B 06")) + 0x2) + 0x4 );
        INIT_XOREDNETVAR_SPECIFIC( m_nCachedBonesRotation,       *(int*)(pattern::find(g_csgo.m_client_dll, XOR("8D 87 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 8B 06")) + 0x25) + 0x4 );
