	...
	void think(Color menu_color) {
		...
		for (size_t i{}; i < m_notify_text.size(); ++i) {
			...
			if (left < .5f) {
				...
				//color.a() = menu_color.a() = (int)(f * 255.f);
			...
			color.a() = menu_color.a() = 180.f;

			std::string prefix = XOR("goofy.solutions ");
			render::FontSize_t fontsize = render::logevent.size(prefix);

			render::esp.string(x, y, menu_color, prefix);
			render::esp.string(x + fontsize.m_width, y, color, notify->m_text);
			y += size;
			//if (i > 14)
			//m_notify_text.erase(m_notify_text.begin() + i); // ghetto m_notify_text.erase(m_notify_text.begin() + i); // ghetto
		}
	}
};
