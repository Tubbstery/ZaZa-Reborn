...
bool Hooks::ShouldDrawFog( ) {
	// remove fog.
	if( g_menu.main.visuals.vis_removals.get(2) && !g_menu.main.visuals.FogOverride.get())
		return false;
...
bool Hooks::DoPostScreenSpaceEffects( CViewSetup* setup ) {
	...
	g_chams.m_in_pse = true;
	g_chams.OnPSE();

	auto og = g_hooks.m_client_mode.GetOldMethod< DoPostScreenSpaceEffects_t >( IClientMode::DOPOSTSPACESCREENEFFECTS )( this, setup );
	g_chams.m_in_pse = false;
